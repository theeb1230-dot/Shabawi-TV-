...
function isCombined(codecs: string): boolean {
  return COMBINED_CODEC_RE.test(codecs);
}

export function parseFormatEntry(f: Record<string, any>, source: string, sigSpec?: CipherSpec | null, playerJs?: string): Format | null {
  ...
  if (!url && cipher) {
    ...
    if (decoded) {
      let sig = decoded.s;
      if (sig && sigSpec) {
        sig = applyCipherSpec(sig, sigSpec);
      }
      try {
  ...
  return {
    ...
    url,
    hasVideo: isVideo,
    hasAudio: isAudio || (isVideo && isCombined(codecs)),
    source,
  };
}
...
export function buildVideoInfo(pr: Record<string, any>, videoId: string, formats: Format[]): VideoInfo {
  ...
  const thumbs = ((vd.thumbnail as Record<string, any>)?.thumbnails as Record<string, any>[] ?? []).sort(
...
export function deduplicateFormats(formats: Format[]): Format[] {
  ...
  for (const f of formats) {
    ...
    if (!seen.has(key)) {
      ...
      // Prefer format with URL
      const existing = seen.get(key)!;
      if (!existing.url && f.url) seen.set(key, f);
    }
  }
  return [...seen.values()];
}

export function printFormatsTable(formats: Format[]): void {
  const header = "ID\t| Quality\t| Resolution\t| Codec\t| Size\t| URL";
  ...
  for (const f of formats) {
    ...
    const size = f.sizeLabel ?? "-";
    console.log(`${f.id}\t| ${f.quality}\t| ${res}\t| ${f.codec.slice(0, 20)}\t| ${size}\t| ${hasUrl}`);
  }
  console.log(sep);
  console.log(`Total: ${formats.length} formats, ${formats.filter((f) => f.url).length} with direct URLs`);
}
